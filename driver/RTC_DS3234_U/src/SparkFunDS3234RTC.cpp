...
SparkFun_DS3234::SparkFun_DS3234(uint8_t cs, uint8_t mosi, uint8_t miso, uint8_t sck)
{
...
SparkFun_DS3234::SparkFun_DS3234(uint8_t cs, SPIClass *spi)
{
  for (int i=0; i<TIME_ARRAY_LENGTH; i++)
  {
    _time[i] = 0;
...
void SparkFun_DS3234::setTime(uint8_t sec, uint8_t min, uint8_t hour, uint8_t day, uint8_t date, uint8_t month, uint8_t year)
{
...
void SparkFun_DS3234::setTime(uint8_t sec, uint8_t min, uint8_t hour12, bool pm, uint8_t day, uint8_t date, uint8_t month, uint8_t year)
{
	_time[TIME_SECONDS] = DECtoBCD(sec);
	_time[TIME_MINUTES] = DECtoBCD(min);
...
void SparkFun_DS3234::setTime(uint8_t * time, uint8_t len)
{
...
void SparkFun_DS3234::update(void)
{
...
uint8_t SparkFun_DS3234::getSecond(void)
{
...
uint8_t SparkFun_DS3234::getMinute(void)
{
...
uint8_t SparkFun_DS3234::getHour(void)
{
...
uint8_t SparkFun_DS3234::getDay(void)
{
...
uint8_t SparkFun_DS3234::getDate(void)
{
...
uint8_t SparkFun_DS3234::getMonth(void)
{
...
uint8_t SparkFun_DS3234::getYear(void)
{
...
void SparkFun_DS3234::setSecond(uint8_t s)
{
	if (s <= 59)
	{
...
void SparkFun_DS3234::setMinute(uint8_t m)
{
	if (m <= 59)
	{
...
void SparkFun_DS3234::setHour(uint8_t h)
{
...
void SparkFun_DS3234::setDay(uint8_t d)
{
	if ((d >= 1) && (d <= 7))
	{
...
void SparkFun_DS3234::setDate(uint8_t d)
{
	if (d <= 31)
	{
...
void SparkFun_DS3234::setMonth(uint8_t mo)
{
	if ((mo >= 1) && (mo <= 12))
	{
...
void SparkFun_DS3234::setYear(uint8_t y)
{
	if (y <= 99)
	{
...
void SparkFun_DS3234::set12Hour(bool enable12)
{
	if (enable12)
		set24Hour(false);
	else
...
void SparkFun_DS3234::set24Hour(bool enable24)
{
	uint8_t hourRegister = spiReadByte(DS3234_REGISTER_HOURS);
...
bool SparkFun_DS3234::is12Hour(void)
{
	uint8_t hourRegister = spiReadByte(DS3234_REGISTER_HOURS);
...
void SparkFun_DS3234::enable(void)
{
...
void SparkFun_DS3234::disable(void)
{
	uint8_t controlRegister = spiReadByte(DS3234_REGISTER_CONTROL);
...
void SparkFun_DS3234::setAlarm1(uint8_t second, uint8_t minute, uint8_t hour, uint8_t date, bool day)
{
...
void SparkFun_DS3234::setAlarm1(uint8_t second, uint8_t minute, uint8_t hour12, bool pm, uint8_t date, bool day)
{
	uint8_t alarmRegister[4];
...
// By default the "date" value is the day-of-month (1-31)
// The "day" boolean changes the "date" value to a day (between 1-7).
void SparkFun_DS3234::setAlarm2(uint8_t minute, uint8_t hour, uint8_t date, bool day)
{
...
void SparkFun_DS3234::setAlarm2(uint8_t minute, uint8_t hour12, bool pm, uint8_t date, bool day)
{	
	uint8_t alarmRegister[3];
...
bool SparkFun_DS3234::alarm1(bool clear)
{
...
bool SparkFun_DS3234::alarm2(bool clear)
{
	uint8_t statusRegister = spiReadByte(DS3234_REGISTER_STATUS);
...
void SparkFun_DS3234::enableAlarmInterrupt(bool alarm1, bool alarm2)
{
	uint8_t controlRegister = spiReadByte(DS3234_REGISTER_CONTROL);
...
//
void SparkFun_DS3234::controlAlarmInterrupt(int alarm1, int alarm2)
{
  uint8_t intch_bit, a2ie_bit, a1ie_bit, other_bits;
  uint8_t controlRegister = spiReadByte(DS3234_REGISTER_CONTROL);
  intch_bit  = controlRegister & 0b100;
  a2ie_bit   = controlRegister & 0b10;
  a1ie_bit   = controlRegister & 0b1;
  other_bits = controlRegister & 0b11111000;
  // alarm1
  if ((alarm1 == 1) || ((alarm1 == -1) && (a1ie_bit > 0))) {
    a1ie_bit = 0b1;
  } else {
    a1ie_bit = 0;
  }
  // alarm2
  if ((alarm2 == 1) || ((alarm2 == -1) && (a2ie_bit > 0))) {
    a2ie_bit = 0b10;
  } else {
    a2ie_bit = 0;
  }
  // intch_bit
  intch_bit = 0b100;
  controlRegister = other_bits | intch_bit | a2ie_bit | a1ie_bit;
  spiWriteByte(DS3234_REGISTER_CONTROL, controlRegister);
}

//
bool SparkFun_DS3234::validateTimeData(void)
{
  uint8_t controlRegister = spiReadByte(DS3234_REGISTER_CONTROL);
  uint8_t osf_bit = controlRegister & OSF_BIT;
  controlRegister = controlRegister & 0b01111111;
  spiWriteByte(DS3234_REGISTER_CONTROL, controlRegister);
  if ( 0 < osf_bit ) {
    return false;
  }
...
void SparkFun_DS3234::writeSQW(sqw_rate value)
{
	uint8_t controlRegister = spiReadByte(DS3234_REGISTER_CONTROL);
...
float SparkFun_DS3234::temperature(void)
{
	float retVal = 0;
	int8_t integer;
...
uint8_t SparkFun_DS3234::BCDtoDEC(uint8_t val)
{
	return ( ( val / 0x10) * 10 ) + ( val % 0x10 );
}

// BCDtoDEC -- convert decimal to binary-coded decimal (BCD)
uint8_t SparkFun_DS3234::DECtoBCD(uint8_t val)
{
	return ( ( val / 10 ) * 0x10 ) + ( val % 10 );
}

uint8_t SparkFun_DS3234::softXfer(uint8_t x) {
  // software SPI
  uint8_t reply = 0;
...
void SparkFun_DS3234::spiWriteBytes(DS3234_registers reg, uint8_t * values, uint8_t len)
{
	uint8_t writeReg = reg | 0x80;

  if (0xFF == _sck ) {
  	_spi->beginTransaction(DS3234SPISettings);
  	digitalWrite(_cs, LOW);
	  _spi->transfer(writeReg);
	  for (int i=0; i<len; i++)
	  {
    ...
    //softXfer(reg & ~0x80); // write, bit 7 low
    softXfer(reg ); // write, bit 7 low
    for (int i=0; i<len; i++)
    {
      softXfer(values[i]);
    }
...
void SparkFun_DS3234::spiWriteByte(DS3234_registers reg, uint8_t value)
{
	uint8_t writeReg = reg | 0x80;
...
uint8_t SparkFun_DS3234::spiReadByte(DS3234_registers reg)
{
  uint8_t retVal = 0;
  if (0xFF == _sck) {
  	_spi->beginTransaction(DS3234SPISettings);
    ...
    digitalWrite(_cs, LOW);
    //softXfer(reg | 0x80); // read, bit 7 high
    softXfer(reg); // read, bit 7 high
    retVal = softXfer(0);
    digitalWrite(_cs, HIGH);
  }
	return retVal;
...
void SparkFun_DS3234::spiReadBytes(DS3234_registers reg, uint8_t * dest, uint8_t len)
{
  if (0xFF == _sck) {
  	_spi->beginTransaction(DS3234SPISettings);
	  digitalWrite(_cs, LOW);
...
void SparkFun_DS3234::writeToSRAM(uint8_t address, uint8_t data){
  spiWriteByte(DS3234_REGISTER_SRAMA, address);
  spiWriteByte(DS3234_REGISTER_SRAMD, data);
}

uint8_t SparkFun_DS3234::readFromSRAM(uint8_t address){
  spiWriteByte(DS3234_REGISTER_SRAMA, address);
  return spiReadByte(DS3234_REGISTER_SRAMD);
}

//void SparkFun_DS3234::writeToRegister(uint8_t address, uint8_t data)
void SparkFun_DS3234::writeToRegister(DS3234_registers address, uint8_t data)
{
   spiWriteByte(address, data);
}

//uint8_t SparkFun_DS3234::readFromRegister(uint8_t address)
uint8_t SparkFun_DS3234::readFromRegister(DS3234_registers address)
{
