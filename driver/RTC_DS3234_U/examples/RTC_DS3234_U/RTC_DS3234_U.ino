...
#define TEST_TEMPERATURE_COLS 3
...
// Mega
//RTC_DS3234_U rtc = RTC_DS3234_U(53,&SPI);
// Due
//RTC_DS3234_U rtc = RTC_DS3234_U(10,&SPI);
// MKR WiFi 1010
//RTC_DS3234_U rtc = RTC_DS3234_U(6,&SPI);
// ESP32
...
void testSetDate(void) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_MINUTES, 0b01111111, min)) {
      Serial.println(F("1 Error: Minute reg (0x01) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_HOURS, 0b00111111, hour)) {
      Serial.println(F("1 Error: Hour reg (0x02) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_DAY, 0b00000111, wday)) {
      Serial.println(F("1 Error: Day reg (0x03) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_DATE, 0b00111111, mday)) {
      Serial.println(F("1 Error: Weekday reg (0x04) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_MONTH, 0b00011111, month)) {
      Serial.println(F("1 Error: Month reg (0x05) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_YEAR, 0b11111111, year)) {
...
void testFreq(void) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00011100, 0b00001000)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL,  0b00011100, 0b00011000)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00000100, 0b00000100)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    Serial.println(F(" register        | val(bin) "));
    Serial.println(F(" ------------+---+--------- "));
    Serial.println(F(" Status      | F | xxxx0xxx "));
    //
    if (!rtc.checkRegValues(DS3234_REGISTER_STATUS, 0b00001000, 0b00000000)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    if (!rtc.checkRegValues(DS3234_REGISTER_STATUS, 0b00001000, 0b00001000)) {
...
void testAlarm(void) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_STATUS, 0b00000011, 0b00000000)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_A1SEC, 0b11111111, sec)) {
      Serial.println(F("3.2 Error: Alarm 1 Second reg (0x07) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_A1MIN, 0b11111111, min)) {
      Serial.println(F("3.2 Error: Alarm 1 Minute reg (0x08) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_A1HR, 0b10000000, 0b10000000)) {
      Serial.println(F("3.2 Error: Alarm 1 Hour reg (0x09) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_A1DA, 0b10000000, 0b10000000)) {
      Serial.println(F("3.2 Error: Alarm 1 week/day reg (0x0A) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00000101, 0b00000101)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_STATUS, 0b00000001, 0b00000001)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00000001, 0b00000000)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    Serial.println(F(" register             | val(bin) "));
    Serial.println(F(" -----------------+---+--------- "));
    Serial.println(F(" control          | E | xxxxx1x0"));
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00000101, 0b00000100)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_A2MIN, 0b11111111, min)) {
      Serial.println(F("3.8 Error: Alarm 2 Minute reg (0x0B) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_A2HR, 0b10000000, 0b10000000)) {
      Serial.println(F("3.8 Error: Alarm 2 Hour reg (0x0C) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_A2DA, 0b10000000, 0b10000000)) {
      Serial.println(F("3.8 Error: Alarm 2 week/day reg (0x0D) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00000110, 0b00000110)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_STATUS, 0b00000011, 0b00000010)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    Serial.println(F(" register             | val(bin) "));
    Serial.println(F(" -----------------+---+--------- "));
    Serial.println(F(" control          | E | xxxxx10x"));
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00000110, 0b00000100)) {
...
void testTemperature(void){
  ...
  // 温度制御機能の設定
  Serial.println(F("4.2 set temperature control function parameter."));
  // CRATE1 , CRATE0 : 0 , 1
  // BB_TD : 1
  int rst = rtc.setTemperatureFunction(0b11);
  if (rst<0) {
    Serial.print(F("Error : set temperature control function parameter, error="));Serial.println(rst);
  } else {
    Serial.println(F("Success : set temperature control function parameter."));
  }
#ifdef DEBUG
  if (!rtc.backupRegValues()) {
    Serial.println(F("Error: fail to get register values."));
    ...
    Serial.println(F(" register              | val(bin) "));
    Serial.println(F(" -----------------+----+--------- "));
    Serial.println(F(" status           |  F | xx01xxxx"));
    Serial.println(F(" control          | 13 | 00000001"));
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_STATUS, 0b00110000, 0b00010000)) {
      Serial.println(F("4.2 Error: Status reg (0x0F) is unmatch."));
      flag=false;
    }
    if (!rtc.checkRegValues(DS3234_REGISTER_TEMPEN, 0b00000001, 0b00000001)) {
      Serial.println(F("4.2 Error: Temperature EN reg (0x13) is unmatch."));
      flag=false;
    }
    if (flag) {
      Serial.println(F("4.2 Success"));
      testResults[TEST_TEMPERATURE][TEST_TEMPERATURE_PARAM]=TEST_SUCCESS;
    } else {
      Serial.println(F("4.2 Fail"));
      testResults[TEST_TEMPERATURE][TEST_TEMPERATURE_PARAM]=TEST_FAIL;
    }
  }
#ifdef DUMP_REGISTER
  rtc.dumpReg();
  ...
  // 手動でTCXOを実行
  Serial.println(F("4.3 do TCXO function manually."));
  rst = rtc.controlTemperatureFunction(1);
  if (rst<0) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_CONTROL, 0b00100000, 0b00100000)) {
      Serial.println(F("4.3 Error: Control reg (0x0E) is unmatch."));
      testResults[TEST_TEMPERATURE][TEST_TEMPERATURE_EXEC_TCXO]=TEST_FAIL;
    } else {
      Serial.println(F("4.3 Success"));
...
void testOscillatorControl(void) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_XTAL, 0b11111111, 0b01010101)) {
  ...
  if (!rtc.backupRegValues()) {
    ...
    //
    bool flag=true;
    if (!rtc.checkRegValues(DS3234_REGISTER_XTAL, 0b11111111, 0b10101010)) {
